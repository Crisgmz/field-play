-- ============================================================
-- MIGRATION 012: Aceptar payment_method='card' (tarjeta en oficina)
-- ============================================================
-- CONTEXT:
--   El cliente ahora puede elegir 3 formas de pago al reservar:
--     * 'bank_transfer' → sube comprobante (ya existe)
--     * 'cash'          → paga en efectivo al llegar al club
--     * 'card'          → paga con tarjeta en el POS del club
--   Las tres dejan la reserva en estado 'pending'. Cash y card no
--   requieren subir comprobante: el admin confirma manualmente
--   cuando el cliente paga en oficina.
--
-- HOW TO APPLY:
...
-- 2) Actualizar el RPC para aceptar 'card' --------------------
-- Mantenemos la misma firma — solo cambia la lista de valores válidos.
...
begin
  ...
  -- Aceptamos las 3 formas de pago.
...
--   -- Debe incluir 'bank_transfer', 'cash', 'card'.
